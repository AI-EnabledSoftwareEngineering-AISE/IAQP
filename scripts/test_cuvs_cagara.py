...
def build_cagra_index_wrapper(X_gpu: cp.ndarray, T_train_index_tune: np.ndarray, 
    ...
    if _tuning is not None:
        ...
        if GT_original_index_tune is None:
            print("🟡 No cached GT data found. Computing GT for tuning...")
            ...
            print(f"shape of GT_original_index_tune: {GT_original_index_tune.shape}")
            print("✅ GT for original queries ready for index tuning")
            
            # Save GT data for future use
            save_gt_data(pkl_path, GT_original_index_tune)
        else:
            print(f"✅ Loaded cached GT data with shape: {GT_original_index_tune.shape}")
